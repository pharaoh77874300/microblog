...
actor {
  ...
  var bookmarks : Map.Map<Principal, Map.Map<Nat, Time.Time>> = Map.empty();
  var directMessages : Map.Map<Nat, DirectMessage> = Map.empty();
  var nextMessageId : Nat = 0;
  // conversationKey = (minPrincipal, maxPrincipal) - we use a per-user map keyed by other user
  var userConversations : Map.Map<Principal, Map.Map<Principal, Nat>> = Map.empty(); // user -> otherUser -> lastMessageId
  ...
  func toPostResponse(post : Post, caller : ?Principal) : PostResponse {
    ...
    let isBookmarked = switch (caller) {
      case (?c) {
        switch (bookmarks.get(c)) {
  ...
  public shared ({ caller }) func bookmarkPost(postId : Nat) : async () {
    requireAuth(caller);
    switch (posts.get(postId)) {
      case (null) { Runtime.trap("Post not found") };
      case (?_) {};
    ...
    let userBookmarks = switch (bookmarks.get(caller)) {
      case (?m) { m };
      case (null) {
        let m = Map.empty<Nat, Time.Time>();
        bookmarks.add(caller, m);
        m;
      };
    };
    userBookmarks.add(postId, Time.now());
  };

  public shared ({ caller }) func unbookmarkPost(postId : Nat) : async () {
    requireAuth(caller);
  ...
  public query ({ caller }) func getBookmarks(cursor : ?Nat, limit : Nat) : async PaginatedPosts {
    requireAuth(caller);
    let effectiveLimit = if (limit == 0 or limit > 50) { 20 } else { limit };
    let userBookmarks = switch (bookmarks.get(caller)) {
      case (?m) { m };
    ...
    // Collect all bookmarked post ids sorted descending by bookmark time
    let entries = List.empty<(Nat, Time.Time)>();
    for ((postId, ts) in userBookmarks.entries()) {
      entries.add((postId, ts));
    };
    entries.sortInPlace(func(a, b) {
      if (a.1 > b.1) { #less } else if (a.1 < b.1) { #greater } else { #equal }
    });

    let startTs : ?Time.Time = switch (cursor) {
      case (?c) {
        switch (userBookmarks.get(c)) {
          case (?ts) { ?ts };
          case (null) { null };
        };
      };
    ...
    var skipping = startTs != null;
    let buf = List.empty<PostResponse>();
    var lastId : ?Nat = null;
    var hasMore = false;

    for ((postId, ts) in entries.values()) {
      if (skipping) {
        switch (startTs) {
          case (?sts) { if (ts < sts) { skipping := false } };
          case (null) { skipping := false };
        };
      };
      if (not skipping) {
        if (buf.size() < effectiveLimit) {
    ...
    {
      posts = buf.toArray();
      nextCursor = if (hasMore) { lastId } else { null };
      hasMore;
    };
  };
  ...
  public shared ({ caller }) func sendMessage(recipient : Principal, text : Text) : async DirectMessage {
    ...
    if (text.size() > 1000) { Runtime.trap("Message must be 1000 characters or fewer") };
    if (caller == recipient) { Runtime.trap("Cannot message yourself") };
    if (hasBlocked(recipient, caller)) { Runtime.trap("Cannot send message") };
    ...
    let callerConvos = switch (userConversations.get(caller)) {
      case (?m) { m };
      case (null) {
        let m = Map.empty<Principal, Nat>();
        userConversations.add(caller, m);
        m;
      };
    };
    callerConvos.add(recipient, id);

    let recipientConvos = switch (userConversations.get(recipient)) {
      case (?m) { m };
      case (null) {
        let m = Map.empty<Principal, Nat>();
        userConversations.add(recipient, m);
        m;
      };
    };
    recipientConvos.add(caller, id);

    msg;
  };

  public query ({ caller }) func getConversations() : async [ConversationResponse] {
    requireAuth(caller);
    let convos = switch (userConversations.get(caller)) {
      case (?m) { m };
      case (null) { return [] };
    };

    let results = List.empty<ConversationResponse>();
    for ((otherPrincipal, lastMsgId) in convos.entries()) {
      switch (directMessages.get(lastMsgId)) {
        case (?lastMsg) {
          switch (userProfiles.get(otherPrincipal)) {
            case (?otherProfile) {
              // Count unread from other user
              var unreadCount : Nat = 0;
              for ((_, msg) in directMessages.entries()) {
                if (msg.sender == otherPrincipal and msg.recipient == caller and not msg.isRead) {
                  unreadCount += 1;
                };
              };
              results.add({
                ...
                otherProfilePictureHash = otherProfile.profilePictureHash;
  ...
  public query ({ caller }) func getMessages(otherPrincipal : Principal, cursor : ?Nat, limit : Nat) : async PaginatedMessages {
    requireAuth(caller);
    let effectiveLimit = if (limit == 0 or limit > 50) { 30 } else { limit };
    let start = switch (cursor) {
      case (?c) { c };
      case (null) { nextMessageId };
    };
    if (start == 0) {
      return { messages = []; nextCursor = null; hasMore = false };
    };

    let buf = List.empty<DirectMessage>();
    let startInt : Int = start;
    for ((_, msg) in directMessages.reverseEntriesFrom(Int.abs(startInt - 1))) {
      let isConvMsg = (msg.sender == caller and msg.recipient == otherPrincipal) or
                      (msg.sender == otherPrincipal and msg.recipient == caller);
      if (isConvMsg) {
        if (buf.size() < effectiveLimit) {
          buf.add(msg);
        } else {
          let arr = buf.toArray();
          return { messages = arr; nextCursor = ?arr[arr.size() - 1].id; hasMore = true };
        };
      };
    };
    { messages = buf.toArray(); nextCursor = null; hasMore = false };
  };

  public shared ({ caller }) func markConversationRead(otherPrincipal : Principal) : async () {
    requireAuth(caller);
    for ((id, msg) in directMessages.entries()) {
      if (msg.sender == otherPrincipal and msg.recipient == caller and not msg.isRead) {
        directMessages.add(id, {
          id = msg.id;
          sender = msg.sender;
  ...
  public query ({ caller }) func getUnreadMessageCount() : async Nat {
    ...
    var count : Nat = 0;
    for ((_, msg) in directMessages.entries()) {
      if (msg.recipient == caller and not msg.isRead) {
        count += 1;
      };
    };
    count;
