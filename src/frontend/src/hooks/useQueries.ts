...
// ===================== Extended Actor Interface (DM + Bookmarks) =====================
...
interface ExtendedBackend {
  bookmarkPost(postId: bigint): Promise<void>;
  unbookmarkPost(postId: bigint): Promise<void>;
  getBookmarks(cursor: bigint | null, limit: bigint): Promise<PaginatedPosts>;
  sendMessage(recipient: Principal, text: string): Promise<DirectMessage>;
  getConversations(): Promise<Array<ConversationResponse>>;
  getMessages(
    otherPrincipal: Principal,
    cursor: bigint | null,
    limit: bigint,
  ): Promise<PaginatedMessages>;
  markConversationRead(otherPrincipal: Principal): Promise<void>;
  getUnreadMessageCount(): Promise<bigint>;
}

function asExtended(actor: unknown): ExtendedBackend {
  return actor as ExtendedBackend;
}
...
export function useBookmarks() {
  ...
  return useInfiniteQuery({
    ...
    queryFn: async ({ pageParam }) => {
      if (!actor) throw new Error("Actor not ready");
      return asExtended(actor).getBookmarks(pageParam, DEFAULT_PAGE_SIZE);
    },
    initialPageParam: null as bigint | null,
    getNextPageParam: (lastPage) => lastPage.nextCursor ?? undefined,
    enabled: !!actor && !isFetching,
...
export function useBookmarkPost() {
  ...
  return useMutation({
    mutationFn: async (postId: bigint) => {
      if (!actor) throw new Error("Actor not ready");
      await asExtended(actor).bookmarkPost(postId);
    },
    onSuccess: () => {
...
export function useUnbookmarkPost() {
  ...
  return useMutation({
    mutationFn: async (postId: bigint) => {
      if (!actor) throw new Error("Actor not ready");
      await asExtended(actor).unbookmarkPost(postId);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["bookmarks"] });
      queryClient.invalidateQueries({ queryKey: ["globalFeed"] });
...
export function useConversations() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      if (!actor) throw new Error("Actor not ready");
      return asExtended(actor).getConversations();
    },
    enabled: !!actor && !isFetching,
...
export function useMessages(otherPrincipal: string) {
  const { actor, isFetching } = useActor();
  ...
  return useInfiniteQuery({
    ...
    queryFn: async ({ pageParam }) => {
      ...
      const principal = P.fromText(otherPrincipal);
      return asExtended(actor).getMessages(
        principal,
        pageParam,
        DEFAULT_PAGE_SIZE,
      );
...
export function useUnreadMessageCount() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      if (!actor) throw new Error("Actor not ready");
      return asExtended(actor).getUnreadMessageCount();
    },
    enabled: !!actor && !isFetching,
    refetchInterval: 30_000,
  });
...
export function useMarkConversationRead() {
  ...
  return useMutation({
    mutationFn: async (otherPrincipal: Principal) => {
      if (!actor) throw new Error("Actor not ready");
      await asExtended(actor).markConversationRead(otherPrincipal);
    },
    onSettled: () => {
...
export function useSendMessage() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (!actor) throw new Error("Actor not ready");
      return asExtended(actor).sendMessage(recipient, text);
    },
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({
        queryKey: ["messages", variables.recipient.toText()],
      });
      queryClient.invalidateQueries({ queryKey: ["conversations"] });
    },
  });
