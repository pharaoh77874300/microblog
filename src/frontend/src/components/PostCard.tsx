...
export function PostCard({ post }: PostCardProps) {
  ...
  const { mutate: unbookmarkPost, isPending: isUnbookmarking } =
    useUnbookmarkPost();
  const isBookmarkPending = isBookmarking || isUnbookmarking;

  const handleBookmarkToggle = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (actionPost.isBookmarkedByCurrentUser) {
      unbookmarkPost(actionPost.id);
    } else {
      bookmarkPost(actionPost.id);
    }
  };
              ...
              >
                ...
                {!!currentPrincipal && (
                  <button
                    ...
                    className={cn(
                      "flex items-center gap-1 text-sm transition-colors",
                      actionPost.isBookmarkedByCurrentUser
                        ? "text-primary"
